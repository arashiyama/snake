...
def is_move_safe(new_head, snake, fruit, traps):
    ...
    In this simulation, traps are treated as obstacles.
    """
    # If the move lands on a trap, it's not safe.
...
def flood_fill_count(start, snake, traps):
    ...
    of reachable cells. The obstacles are the snake's body and the traps.
    """
    obstacles = set(snake)
    obstacles.update(traps)
    visited = set()
...
def get_safe_move(snake, fruit, traps):
    """
    Fallback strategy: from the snake's head, evaluate all four directions.
    For each candidate that is immediately safe (no collision with wall, snake,
    or trap) and passes the connectivity test, compute the flood-fill area (free space).
    Return the move that maximizes the free space available.
    
    If none pass the connectivity test, return any immediate safe move.
    """
    head = snake[0]
    candidate_moves = []
    ...
    for dx, dy in directions:
        ...
        if not is_move_safe(new_head, snake, fruit, traps):
            continue
        # Compute free area available from new_head.
        area = flood_fill_count(new_head, snake, traps)
        candidate_moves.append((new_head, area))
    
    if candidate_moves:
        # Choose the move that maximizes the flood-fill count.
    ...
    for dx, dy in directions:
        ...
        if new_head in snake:
            continue
        return new_head
    return head  # No safe move found; will result in collision.

# -------- Main Game Loop --------
def main():
    pygame.init()
    screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
    pygame.display.set_caption("Self-Playing Snake Game (Improved Self-Avoidance with Traps)")
    ...
    while running:
        ...
        if path and len(path) >= 2:
            ...
            # Check connectivity safety with traps included.
            if not is_move_safe(candidate_move, snake, fruit, traps):
                candidate_move = get_safe_move(snake, fruit, traps)
            next_move = candidate_move
        else:
            # If no path to the fruit is found, choose a safe move using the flood fill heuristic.
            next_move = get_safe_move(snake, fruit, traps)
