...
def is_move_safe(new_head, snake, fruit):
    ...
    The simulation considers that if the snake is not eating a fruit,
    its tail will be removed. To account for this, we treat the tail cell
    as free during the connectivity check.
    """
    ...
    if new_head == fruit["pos"]:
        ...
        # Not eating fruit: snake moves (tail is removed)
        new_snake = [new_head] + snake[:-1]
    
    # For connectivity check, treat all cells of new_snake except the tail as obstacles.
    ...
    while queue:
        ...
        for neighbor in get_neighbors(current):
            ...
            if x < 0 or x >= GRID_WIDTH or y < 0 or y >= GRID_HEIGHT:
                continue
            # Even if neighbor equals target, it is acceptable.
            if neighbor == target:
                return True
...
def get_safe_move(snake, fruit):
    ...
    If no candidate passes the connectivity check, return any immediate safe move.
    """
    head = snake[0]
    ...
    for dx, dy in directions:
        ...
        if is_move_safe(new_head, snake, fruit):
            return new_head
    # If none of the moves passes the connectivity test, choose any immediate safe move.
...
def main():
    ...
    pygame.display.set_caption("Self-Playing Snake Game (Avoiding Self-Collision)")
    ...
    while running:
        ...
        if path and len(path) >= 2:
            ...
            # Before committing to the candidate move, check if it is safe in the long run.
            if not is_move_safe(candidate_move, snake, fruit):
                candidate_move = get_safe_move(snake, fruit)
            next_move = candidate_move
        else:
            # If no path to the fruit is found, choose a safe move.
